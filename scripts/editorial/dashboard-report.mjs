...
const queueRows = actionQueue.map((item, index) => `
  <tr data-priority="${escapeHtml(item.priority)}" data-status="${escapeHtml(item.status)}" data-search="${escapeHtml(`${item.title} ${item.file} ${item.collection}`.toLowerCase())}">
    ...
    <td><strong>${escapeHtml(item.title || item.slug)}</strong><small>${escapeHtml(item.file)}</small></td>
...
<div class="card kpi"><span>Stale</span><strong>${status.stale ?? 0}</strong><span>priorità di revisione</span></div>
<div class="card kpi"><span>Undated</span><strong>${status.undated ?? 0}</strong><span>data non disponibile</span></div>
...
<div class="card full"><h2>Coda operativa P1/P2</h2><div class="controls"><input id="search" type="search" placeholder="Cerca titolo, file o collezione" aria-label="Cerca nella coda operativa"><select id="priority" aria-label="Filtra per priorità"><option value="">Tutte le priorità</option><option>P1</option><option>P2</option></select><select id="status" aria-label="Filtra per stato"><option value="">Tutti gli stati</option><option value="stale">stale</option><option value="review-due">review-due</option><option value="undated">undated</option></select></div><div class="table-wrap"><table id="queue"><thead><tr><th>#</th><th>Priorità</th><th>Stato</th><th>Contenuto</th><th>Collezione</th><th>Riferimento</th><th>Età (gg)</th></tr></thead><tbody>${queueRows}</tbody></table><div id="empty" class="empty">Nessun contenuto corrisponde ai filtri.</div></div></div>
...
<p class="footer">Lab4Int Editorial Dashboard · artefatto locale generato automaticamente</p>
</main>
<script id="dashboard-data" type="application/json">${safeJson(data)}</script>
<script>
const rows=[...document.querySelectorAll('#queue tbody tr')];const search=document.querySelector('#search');const priority=document.querySelector('#priority');const status=document.querySelector('#status');const empty=document.querySelector('#empty');
function filter(){const q=search.value.trim().toLowerCase();let visible=0;for(const row of rows){const ok=(!q||row.dataset.search.includes(q))&&(!priority.value||row.dataset.priority===priority.value)&&(!status.value||row.dataset.status===status.value);row.hidden=!ok;if(ok)visible++}empty.style.display=visible?'none':'block'}
for(const control of [search,priority,status])control.addEventListener('input',filter);
</script>
</body></html>`;
...
console.log(`Esito: ${total} contenuti, ${actionQueue.length} elementi in coda, ${qualitySummary.warnings ?? 0} warning.`);
